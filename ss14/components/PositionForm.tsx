...
type FormData = Omit<Position, "id" | "createAt">;
...
export default function PositionForm({
  onSubmit,
  initialValues = { positionName: "", description: 0, positionStatus: "ACTIVE",createdAt:new Date() },
  ...
  return (
    <View style={styles.container}>
      ...
      <Controller
        ...
        name="positionName"
      />
      {errors.positionName && <Text style={styles.error}>{errors.positionName.message}</Text>}
          ...
          <View style={styles.pickerContainer}>
            <Picker selectedValue={value} onValueChange={onChange}>
              {PositionStatuses.map((status) => (
                <Picker.Item key={status} label={status as string} value={status as string} />
              ))}S
            </Picker>
          </View>
        )}
